...
def extract_keywords(text, limit=20):
    ...
    for w in re.findall(r"[a-z][a-z0-9+#.\-]{1,}", low):
        w = w.rstrip(".-")
        if (len(w) < 3 and w not in ("c#", "go", "ai", "ml", "bi", "qa", "ux", "ui", "hr")) or w in KW_STOP or w.isdigit():
            continue
    ...
    for src in SOURCES:
        ...
        for r in rows:
            ...
            item.update({
                ...
                "keywords": extract_keywords(desc) or prev.get("keywords", []),
                "source": "auto",
            })
            fresh[k] = item
            kept += 1
